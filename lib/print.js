...
console.log(`
╭┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡ GokuBot ⟡┄┄┄┄┄┄┄┄┄┄┄┄┄
╰╮ ${chalk.green('Bot:')} ${chalk.bgMagenta(chalk.white.italic('%s'))}
││ ${chalk.cyan('Hora:')} ${chalk.black.bgWhite('%s')}
││┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄
││ ${chalk.green('Interaccion:')} ${await formatMessageStubType(m.messageStubType, name_user)}
││ ${chalk.green('Usuario:')} ${chalk.white('%s')}
╭╯ %s
╰┄┄┄┄┄┄┄┄┄┄┄┄┄┄⟡ GokuBot ⟡┄┄┄┄┄┄┄┄┄┄┄┄┄
`.trim(),          
me + (conn.user.name == undefined ? '' : ' ~' + conn.user.name) + `${conn.user.jid == global.conn.user.jid ? '' : ' Sub-Bot'}`,
(m.messageTimestamp ? new Date(1000 * (m.messageTimestamp.low || m.messageTimestamp)) : new Date).toTimeString(),
sender,
m ? '' : '',
`${user.money} monedas disponibles.`,
