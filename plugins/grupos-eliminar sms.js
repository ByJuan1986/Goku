...
if (!m.quoted && !m.mentionedJid?.length && !args[0]) return conn.sendMessage(m.chat, {text: `✎ _Ingrese el comando y mensione o responda a un usuario para eliminar su mensaje._`, { quoted: m });
try {
if (m.quoted) {
...
return conn.sendMessage(m.chat, {text: `✎ _Debe de mensionar o responder a alguien para eliminar su mensaje o proporcionar cuantos mensajes hay que eliminar._`, { quoted: m });
}
...
if (!messagesToDelete.length) return conn.sendMessage(m.chat, {text: `✎ _No se ha encontrado los mensajes del participante para eliminar._`, { quoted: m });
