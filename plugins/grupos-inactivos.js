...
switch (command) {
case "inum": 
if(total == 0) return conn.sendMessage(m.chat, {text: `✎ _El grupo no contiene inactivos por el momento, vuelva mas tarde._`, { quoted: m })
...
`.trim()
conn.sendMessage(m.chat, {text: `${inums}`, { quoted: m })
  break   
case "kicknum":  
if(total == 0) return conn.sendMessage(m.chat, {text: `✎ _El grupo no contiene inactivos por el momento, vuelva mas tarde._`, { quoted: m })
...
await conn.sendMessage(m.chat, {text: `${kicknum}`, { quoted: m })
await delay(1 * 10000)
